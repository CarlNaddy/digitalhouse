# Live deployment runbook — digitalhouse
...
| | |
|---|---|
| URL | <https://digitalhouse.fly.dev/> |
| Fly app | `digitalhouse` — this is `fly.toml`'s `app` line; keep every `-a <name>` below in sync with it |
| Region | `ams` (Amsterdam) |
| Database | `digitalhouse-db` — single-node Fly Postgres, same region |
| Admin login | `admin@DigitalHouse.local` + a generated password (see §3 — not reproduced here) |

## 2. Systems and tools involved

| System | Role |
|---|---|
| **This GitHub repo** (`CarlNaddy/dotnet-agentic-starterkit`) | Source of truth for code and for `fly.toml` (the app's runtime config: port, health checks, scaling, the `release_command`) |
| **Fly.io** | Hosts both the app (`digitalhouse`) and its database (`digitalhouse-db`) as separate Fly "apps," each made of one or more Machines |
...
|---|---|---|
| **App secrets** (`ConnectionStrings__Default`, `Seed__AdminPassword`) | Fly's own encrypted secret store, attached to the `digitalhouse` app | `fly secrets list -a digitalhouse` shows *names* only — Fly never returns values back out once set. To change one: `fly secrets set KEY=value -a digitalhouse` (triggers a new release) |
...
dotnet publish DigitalHouse.csproj -t:PublishContainer -c Release \
  ...
  -p:ContainerRepository=digitalhouse \
  -p:ContainerImageTag=<some-tag>
flyctl deploy --image registry.fly.io/digitalhouse:<some-tag> -a digitalhouse
```
...
```bash
flyctl status -a digitalhouse          # app + machine health
flyctl status -a digitalhouse-db       # database health
flyctl logs   -a digitalhouse          # stream app logs (Ctrl+C to stop)
flyctl logs   -a digitalhouse --no-tail  # just the recent buffer, no streaming
flyctl ssh console -a digitalhouse     # shell into a running app Machine
flyctl secrets list -a digitalhouse    # secret NAMES only, never values
flyctl machine restart <id> -a digitalhouse
```
